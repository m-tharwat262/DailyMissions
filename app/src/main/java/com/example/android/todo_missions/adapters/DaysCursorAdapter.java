...
public class DaysCursorAdapter extends CursorAdapter {


    public static final String LOG_TAG = DaysCursorAdapter.class.getSimpleName(); // class name.
    ...
    public DaysCursorAdapter(Context context, Cursor c, int yearNumber, int monthNumber) {
        super(context, c, 0);
    ...
    public void bindView(View view, Context context, final Cursor cursor) {
        ...
        int yearIdInDataBaseColumnIndex = cursor.getColumnIndexOrThrow(DaysEntry.COLUMN_YEAR_ID);
        int monthIdInDataBaseColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_YEAR_ID);
        int dayNameColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_DAY_NAME);
        int dayNumberColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_DAY_NUMBER);
        int dayDescriptionColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_DAY_DESCRIPTION);
        int missionsNumberColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_MISSIONS_NUMBER);
        int iconIdColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_ICON_NUMBER);
        int iconBackgroundIdColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_BACKGROUND_ICON_NUMBER);
        int smallCircleColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_BACKGROUND_SMALL_CIRCLE_NUMBER);
        int unixNumberColumnIndex = cursor.getColumnIndexOrThrow(TodoThingsContract.DaysEntry.COLUMN_UNIX);


        // get the student name from the database and display it in the screen.
        String dayName = cursor.getString(dayNameColumnIndex);
        nameTextView.setText(dayName);

        // get the student name from the database and display it in the screen.
        String dayDate = cursor.getString(dayNumberColumnIndex) + "/" + mMonthNumber + "/" + mYearNumber;
        dateTextView.setText(dayDate);

        // get the student name from the database and display it in the screen.
        int missionsNumber = cursor.getInt(missionsNumberColumnIndex);
        dateDetailsTextView.setText(mContext.getString(R.string.number_of_days, missionsNumber));
